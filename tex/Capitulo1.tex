...
%GENAINA
%For example, means to analyse if the system is able to fulfil the expected level of dependability is not able to be verified via current RGM  proposal. Given the importance of implementing dependable systems, RGM needs to be further extended so that the runtime goal satisfaction is properly analysed.
...
%, through an appropriate verification model and properties defined with linear, temporal order or even probabilistic logic, 
...
%whose ability to avoid failures before they take place is crucial

%Assuming the benefits of a goal-oriented requirements engineering, in this work we investigate the feasibility of a goal-oriented dependability analysis for which system goals are explicitly modelled and their fulfilment must be verified by some appropriate technique.
...
%Following a similar principle of the component-based verification and considering the , this proposal focus on the feasibility of a goal-oriented PMC approach for which the leaf-tasks representing high-level system behaviour are mapped to a probabilistic model to provide both qualitative and quantitative analysis for the probability of successfully fulfilling different system goals. 

%Given a correct and consistent runtime goal model for which goals are ultimately satisfied by system tasks and a set of leaf-tasks that fulfils some specific system goal, is it possible to analyse the probability of achieving this goal through a probabilistic model checking technique?

\bigskip

%ultimately satisfied by the successful execution of a

\setlength{\fboxsep}{10pt}
\noindent\fbox{%
    \parbox{0.95\textwidth}{%
        \textbf{Research Question 1 (RQ1):}	Given a correct and consistent runtime goal model for which goals are realized by a set of leaf-tasks, is it feasible to analyse the probability of achieving one or more system goals through a probabilistic model checking technique?
    }%
}\bigskip
...
\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Research Question 2 (RQ2):}		Given the goal-oriented probabilistic verification in RQ1, 
it feasible to consider the context effects over what goals are required, what alternatives are adoptable and the quality of each alternative?
    }%
}\bigskip


%, i.e., the absence of failures
...
\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Research Question 3 (RQ3):}	Is it feasible and scalable to employ a goal-oriented dependability verification based on parametric PMC as part of a self-adaptation analysis for solving variability at runtime?
    }%
}\bigskip
...
\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Research Question 4 (RQ4):}	Is it possible to automatically generate the probabilistic verification model for the dependability analysis defined in RQ1-2?
    }%
}\bigskip
...
The proposal has been evaluated with the application of the goal-oriented reliability analysis for self-adaptive systems to the development of a Mobile Personal Emergency Response System (MPERS). This system fits in the context of ambient assisted living systems~\cite{Lorincz2004, AAL REF} where a ubiquitous emergency response system runs on a  mobile device~\ref{MPERS:GoSafe}. Instead of a static environment, the MPERS is conceived to allow patients with different health risk degrees to preserve their mobility while they are monitored and assisted. As a mobile system, MPERS is affected by context variations and self-adaptation becomes a mandatory feature to avoid failures, as safety is of paramount importance for this system.
...
%both the analysts (design-time) and the system itself (runtime self-adaptation) with a precise method for reliability verification coped to the goal model specification.
